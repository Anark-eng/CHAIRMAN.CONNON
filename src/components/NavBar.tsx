...
export async function NavBar() {
  ...
  const pendingDeletion = Boolean(profile?.deleted_at);

  return (
    <>
      {user && pendingDeletion && (
        <div className="border-b border-amber-500/40 bg-amber-500/10 px-4 py-2 text-center text-xs">
          Your account is scheduled for deletion. It finalizes 30 days after you asked.{" "}
          <Link href="/settings" className="font-semibold underline">
            Cancel
          </Link>{" "}
          from settings to keep your account.
        </div>
      )}
